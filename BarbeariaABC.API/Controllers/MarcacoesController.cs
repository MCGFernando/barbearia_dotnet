...
namespace BarbeariaABC.API.Controllers
{
    ...
    {
        private readonly IMarcacaoRepository _repo;
        private readonly ILogger<MarcacoesController> _logger;

        public MarcacoesController(IMarcacaoRepository repo, ILogger<MarcacoesController> logger)
        {
            _repo = repo;
            _logger = logger;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Add(Marcacao obj)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] Marcacao obj)
        {
            if (id != obj.Id)
            {
                return BadRequest();
