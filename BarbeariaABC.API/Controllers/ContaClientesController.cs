...
namespace BarbeariaABC.API.Controllers
{
    ...
    public class ContaClientesController : ControllerBase
    {
        ...
        private readonly ILogger<ContaClientesController> _logger;

        public ContaClientesController(IFuncaoRepository repo, ILogger<ContaClientesController> logger)
        {
            _repo = repo;
            _logger = logger;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Add(Funcao obj)
        {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult<Funcao>> Update(int id, Funcao obj)
        {
            if (id != obj.Id)
            {
                return BadRequest();
